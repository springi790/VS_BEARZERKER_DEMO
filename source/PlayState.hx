					...
					case 'bearpark':
						{
							defaultCamZoom = 0.45;
							curStage = 'bearpark';
							var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('bear/bgback', 'shared'));
							bg.antialiasing = true;
							bg.scrollFactor.set(0.9, 0.9);
							bg.active = false;
							add(bg);
	
							var beartower:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('bear/bgtower', 'shared'));
							beartower.antialiasing = true;
							beartower.scrollFactor.set(0.9, 0.9);
							beartower.active = false;
							add(beartower);
				
							var bearground:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('bear/bgground', 'shared'));
							bearground.setGraphicSize(Std.int(bearground.width * 1.1));
							bearground.updateHitbox();
							bearground.antialiasing = true;
							bearground.scrollFactor.set(0.9, 0.9);
							bearground.active = false;
							add(bearground);
	
							var rocks:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('bear/bgrocks', 'shared'));
							...
							var hallowTex = Paths.getSparrowAtlas('bear/cheer_bears', 'shared');
							minis = new FlxSprite();
							minis.updateHitbox();
							minis.frames = hallowTex;
							minis.animation.addByPrefix('idle', 'Cheer Bears');
							minis.animation.play('idle');
							minis.antialiasing = true;
							add(minis);
			}
			case 'stage':
				{
